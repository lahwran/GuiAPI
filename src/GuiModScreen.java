 ...
 * normally you will want to call it's static methods to use it, though subclassing it and/or instantiating it
 * are also possible. however, to do so would use unsafe api (I still might change things.)
 * 
 * @author lahwran
 * @version 0.9.5
 * @see show
...
public class GuiModScreen extends cf {
	/**
	 * reference to parent screen, is used to go back()
	 * @see back()
	 */
	public cf parentScreen;
	...
	/**
	 * only use this constructor from subclasses. does not take a widget, so that the subclass
	 * can build the widget before storing it. put your main widget in mainwidget, of course.
	 * @param by1 parent screen
	 */
	@SuppressWarnings("deprecation")
	protected GuiModScreen(cf by1) 
	{
	 ...
	 */
	public GuiModScreen(cf by1, Widget w)
	{		
	 ...
	 * @param screen widget to show - will be sized to size of screen when twl was started.
	 */
	...
	/**
	 * show a screen - GuiModScreen version. show an instance of GuiModScreen - does not set the parent screen,
	 * you have to deal with that yourself!
	 * @param screen GuiModScreen instance or subclass to show with parent screen already set to current screen.
	 */
	...
	public void a(int j, int k, float f) {
        ...
        //for (int i=0; i<Mouse.getButtonCount(); i++)
        //	GuiWidgetScreen.getInstance().gui.handleMouse(Mouse.getX(), GuiWidgetScreen.screenheight-Mouse.getY(), i, Mouse.isButtonDown(i));
        ...
        for(int i = 0; i < Mouse.getButtonCount(); i++)
        {
        	if (mousepressed[i] != Mouse.isButtonDown(i))
        	{
        		...
        		//System.out.println("Mouse button state change "+i+" to "+mousepressed[i]);
        		if(mousepressed[i])
        		{
        		gui.handleMouse(curMouseX, gui.getHeight() - curMouseY - 1, i, mousepressed[i]);
        		gui.handleMouse(curMouseX, gui.getHeight() - curMouseY - 1, i, !mousepressed[i]);
        		}
        	}
        }
        ...
        {
        	//Mouse.
        	gui.handleMouse(curMouseX, gui.getHeight() - curMouseY - 1,-1, false);
        }
        while(Mouse.next());*/
        LWJGLRenderer r = (LWJGLRenderer) GuiWidgetScreen.getInstance().gui.getRenderer();
        ...
        r.syncViewportSize();
        //GL11.glViewport(0, 0, GuiWidgetScreen.getInstance().mcinstance.c, GuiWidgetScreen.getInstance().mcinstance.d);
        //GuiWidgetScreen.getInstance().layout();
	...
	public void e()
	{
		...
		GuiWidgetScreen.getInstance().gui.handleMouse(
                Mouse.getEventX(), GuiWidgetScreen.getInstance().gui.getHeight() - Mouse.getEventY() - 1,
